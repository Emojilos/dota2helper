...
# GateGuard (fact-forcing hook) вставляет лишний round-trip перед каждым write/bash —
# отключаем его для автономного цикла.
...
2. Перед первым запуском выполни 'npm install'. Проверь, что типы проходят через 'npm run typecheck' и тесты через 'npm test'.
...
5. ОБЯЗАТЕЛЬНО закоммить фичу: `git add -A && git commit`. Это не опция и не «оставить заметку вместо коммита» — коммит должен реально появиться в `git log`. npm/git в этом окружении работают без подтверждений; если коммит падает — почини причину и всё равно закоммить.
